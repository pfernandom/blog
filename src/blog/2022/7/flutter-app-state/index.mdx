...
key_words: "flutter, get_it, rxdart, state management, redux, mobx, android, ios, data stream"
published: true
---

import Image from "src/components/image";
import appstate1 from "./appstate1.gif";
import appstate2 from "./appstate2.gif";
...
At any given moment, an application will have _state_. If we think of an app like Instagram, the state can be
...
<div style={{ width: "50%", margin: "0 auto" }} className="post-image">
  <Image
    src={appstate1}
    alt={"User clicks in the button and a list of notifications is rendered"}
    layout="responsive"
  />
</div>
...
Calling `setState` when we set the new value of the variable `updates` will re-render this widget _only_. If this widget contains other widgets (e.g. the `ElevatedButton` and the `ListView`), those widgets will be re-rendered as well.
...
<div style={{ width: "50%", margin: "0 auto" }} className="post-image">
  <Image
    src={appstate2}
    alt={
      "User clicks in the button, a list of notifications is rendered and the notification icon is rendered as well."
    }
    layout="responsive"
  />
</div>
...
- It may be _too_ flexible. This approach opens the door to a lack of structure in the project. More structure is better for large teams and large code bases.
