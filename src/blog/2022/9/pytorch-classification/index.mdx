...
For a full in-depth explanation of how Neural Networks and backpropagation work, take a look at the free, online book ["Neural Networks and Deep Learning"](http://neuralnetworksanddeeplearning.com/chap2.html).
...
- *One-hot encoding*: Columns that contain categories (e.g. "Sex" contains values like "Male" or "Female") can be converted into numeric values by adding a column for each category, and set a value of 1 for those columns in that category (and 0 for those that don't).
...
|    |   PassengerId |   Survived |   Pclass | Name                                                | Sex    |   Age |   SibSp |   Parch | Ticket           |    Fare | Cabin   | Embarked   | Title   | FamilyS   |
|---:|--------------:|-----------:|---------:|:----------------------------------------------------|:-------|------:|--------:|--------:|:-----------------|--------:|:--------|:-----------|:--------|:----------|
...
- Group columns into more meaningful labels:
  - E.g. add the values `SibSp` and `Parch` into a single category `FamilyS`.
  - Transform `FamilyS` into a category (e.g. `single`, `couple`, `large`).
...
|     | Pclass |   Sex  |  Age |   Fare  | Embarked | Title | FamilyS |
|:---:|:------:|:------:|:----:|:-------:|:--------:|:-----:|:-------:|
...
```python
def encode_data(X):
  categorical_columns = ['Pclass','Sex', 'FamilyS', 'Embarked', 'Title']
...
|   | Age  | Fare    | Pclass_2 | Pclass_3 | Sex_male | FamilyS_InterM | FamilyS_Large | FamilyS_Single | Embarked_Q | Embarked_S | Title_Miss | Title_Mr | Title_Mrs | Title_Rare |
|---|------|---------|----------|----------|----------|----------------|---------------|----------------|------------|------------|------------|----------|-----------|------------|
...
The results of the prediction in `result = net(test_var)` will be a
