...
Learning how to code in [Rust](https://www.rust-lang.org/) can be a daunting task. While it is a very powerful language it is not precisely known for being a simple language.
...
## Error: Trying to modify immutable variables

In the following example, we create an instance of a `struct` called `Book`, and we try to assign a different value to it's `title` property:
...
## Error: Trying to use a "moved" variable
...
## Error: Trying to borrow as both mutable and immutable in the same scope
...
## Error: Trying to keep a reference to a struct value
