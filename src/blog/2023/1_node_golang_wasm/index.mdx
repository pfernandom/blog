...
date: '2023-02-13'
description:
  - In second part of two posts, we will explore how to import a compiled WASM module inside a NodeJS application.
hero_image: ./hero.jpg
...
NodeJS is an excelent tool to quickly set up small services like HTTP servers thanks to its ease of integration with other libraries like SSR for web frameworks and NoSQL database like MongoDB.

However, due its interpreted nature, JavaScript is not the right tool for memory intensive tasks. Other compiled languages like Golang get the upper hand for some tasks, as they are optimized for them. It would be awesome if we could get the best of both worlds, isn't it? Enter **WebAssembly**.

## Brief disclaimer

The example explained in this post -as most WebAssmebly-specific libraries are at this point in time- is based on some **experiment** libraries, which are subject to changes in the future. This doesn't mean you cannot use them in production, but you should be aware that the APIs of these libraries are not guaranteed to be backwards compatible in future releases and extra refactor work may be needed when updating versions of Go or NodeJS.

## Some background

JavaScript is an interpreted language. This means that, due its dynamic typing nature, the application cannot make assumptions about what type of data will be used at run time: A variable passed to a function could be a string, an object or even a function.
...
JIT essentially is a caching strategy over the the code and the specific types used in it. As code is interpreted and executed, the JIT compiler will keep track of the types used in the code, convert it to bytecode (low-level instructions) and use this optimized version of the code next time this code is called.

While JIT performs many other specific improvements, in general this is it's goal: Compile the JavaScript code into optimized instructions. Thanks to JIT compilation, engines like browsers can execute interpreted code at **almost** native code speeds (the keyword being _almost_). And here lays the advantage of compiled languages like Go: Their strongly typed nature (among other language-specific characteristics) allows compilers to perform all optimizations ahead of time, resulting in applications that only need to execute bytecode.
...
- **Portable**. WASM works as a container (or VM), which allows it to compile bytecode that can be run in multiple architectures: Browsers, mobile devices, backend servers; as long as it supports the WASM specification, it can execute WASM code.
...
The same isolation and flexibility that provides most of WASM strengths are the cause of most of its limitations:

- WASM doesn't provide a default memory management (e.g. Garbage Collection) mechanism. This means that, currently, each WASM module needs to be shipped with its own memory management code. (_This is one of the reasons why [**Rust** shines for WASM modules](https://rustwasm.github.io/docs/book/), as its memory management it's baked into the language_)
...
Most WASM examples are simple "Hello World". For this post, I though it was useful to see a more specific use-case, a problem that I've found in the past.
...
First, we create a [NodeJS ReadStream](https://nodejs.org/api/stream.html) using `fs.createWriteStream` to open the `test.log` file in "append" mode. The we start recursively looping using `setTimeout`. This simulates an application that logs text in the file at a variable rate. At each iteration:

- We print some simple text with a random log level from `{'WARN', 'INFO', 'ERROR'}` to the logfile.
...
As I mentioned before, WASM modules cannot output complex types, so if we want to be able to send out something other than numbers or bytes, we need some "glue" code which takes care of mapping types like strings and slices into bytes. The Go repository offers the very convenient [`wasm_exec.js`](https://github.com/golang/go/blob/master/misc/wasm/wasm_exec.js?source=post_page---------------------------), which will allow our WASM module to communicate with the outside world.
...
- It doesn't support custom data structures. This makes sense, as out WASM module would have to export some sort of schema so whoever consumes the module knows out to parse the output. However, `wasm_exect.js` does support slices and maps, which allows us to marshal our Go-defined structures into objects like `map[string]interface{}`.
...
We could use [polling](<https://en.wikipedia.org/wiki/Polling_(computer_science)>) to repeteally call the WASM module to retrieve the latest logs, but this is not an optimal approach, as it requires fine tunning to get the logs in real time without excessive memory consumption.
...
First, we will define the types for our output. For convenience, we will create a new type called `ParsedLogs`, which is just an alias for `map[string]interface{}`.

```go
type ParsedLogs = map[string]interface{}
```

Now, let's define a function which has only one responsibility: Sending the output back to NodeJS. However, since we want to test that everything works without having to compile and deploy the module, for now our callback will just print the parsed logs into console:
...
The `OutputCallback` is another alias we create just for convenience, so we don't have to use the verbose `func(parsedLogs ParsedLogs)` when we pass our function as a paremeter to the main process.
...
It's time to extend the `Execute` function to read our logfile. For this example, we will hard-code the path to the log file, but you could pass this as a parameter, or retrieve it from [a global function defined in NodeJS](https://pkg.go.dev/syscall/js#Global).
...
First, we open our log file, makeing sure we defer it's `close` function:
...
We execute the second loop inside a [goroutine](https://go.dev/tour/concurrency/1), which will allow to have another infinite loop running in parallel.
...
Now, remember we were also calling `Accumulator.Flush` in the infinte loop that reads the output of the `out` channel:
...
The code used to parse the logs is pretty straight forward. It addresses the simple formatting of the log entries in our example, but you can easily extend it to parse more complex patterns:
...
In essense, this library allows us to communicate with the NodeJS code that will execute our WASM module.
...
And then, inside our Go code, we can get a reference to that function like follows:
...
This is the reason why we didn't call `js.Global()` directly inside `Execute`; it decouples most of code from WASM-specific dependencies, which makes them easier to reuse in non-WASM applications.

If we want to execute our code now, we will have to compiled into WebAssmebly bytecode; this can be done with the following command:

```bash
GOOS=js GOARCH=wasm go build -o app.wasm
```

By setting the `GOARCH` environment variable to `wasm`, the module now will compile successfully. In addition, the `GOOS=js` environment variable let's the compiler know we will use this WASM module in JavaScript.
...
One important thing to notice before we end is that executing WASM in NodeJS -instead of a browser like more common examples do- gives us access to host-specific libraries like `fs` to read files and to a memory runtime. All these things are not directly available in browser's JavaScript engines, which means the WASM module we created in this example would not run there.
...
WebAssembly has a promising future as a standard to modularize interoperable code. We can expect that future improvements around the specification (especially with WASI) will open the door to use WASM for creating embedable containers of functionality that can be imported in almost any environment from the browser and backend servers to edge-computing devices like those in IoT.
